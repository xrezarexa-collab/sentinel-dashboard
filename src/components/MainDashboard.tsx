...
import { CloudRainWind, ThermometerSun, Wallet, Activity, RefreshCw, Layers } from "lucide-react";

export default function MainDashboard() {
  const [balance, setBalance] = useState("$8,000.00");
  const [profit, setProfit] = useState("+$7,900.00");
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/5 border border-white/10 p-6 rounded-3xl backdrop-blur-xl">
          ...
          <div className="flex items-center gap-3">
             ...
             <button className="p-2 bg-white/5 hover:bg-white/10 rounded-xl transition border border-white/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white/5 border border-white/10 p-6 rounded-3xl backdrop-blur-xl relative overflow-hidden">
             ...
             <div className="flex items-center gap-3 mb-2">
                ...
                <h3 className="text-gray-400 font-medium">Net Profit (Weather)</h3>
             </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-white/5 border border-white/10 rounded-3xl backdrop-blur-xl overflow-hidden flex flex-col h-[600px]">
          <div className="p-4 border-b border-white/10 flex items-center justify-between bg-black/20">
             ...
             <span className="text-xs bg-blue-500/20 text-blue-400 px-3 py-1 rounded-full border border-blue-500/30">Server: 95.111.247.105:8080</span>
          </div>
          <div className="flex-1 bg-black/40 p-0 relative">
             ...
             {/* Note: IFrame to a different origin or non-ssl origin might be blocked by browsers, but works locally for self-hosted */}
             <iframe 
                src="http://95.111.247.105:8080" 
                className="w-full h-full border-0 relative z-10 mix-blend-screen opacity-90"
